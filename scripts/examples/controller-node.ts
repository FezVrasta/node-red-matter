...
const storage = new StorageBackendDisk(
  storageLocation,
...
class ControllerNode {
  async start() {
    ...
    const ip = controllerStorage.has('ip')
    ...
    if (pairingCode !== undefined) {
      ...
      logger.debug(
        `Data extracted from pairing code: ${Logger.toJSON(pairingCodeCodec)}`
      );
    } else {
      longDiscriminator =
        getIntParameter('longDiscriminator') ??
        controllerStorage.get('longDiscriminator', 3840);
      if (longDiscriminator > 4095)
        throw new Error('Discriminator value must be less than 4096');
      setupPin =
        getIntParameter('pin') ?? controllerStorage.get('pin', 20202021);
    }
    if (
      (shortDiscriminator === undefined && longDiscriminator === undefined) ||
      setupPin === undefined
    ) {
      throw new Error(
    ...
    try {
      ...
      const descriptor =
        commissioningController.getRootClusterClient(DescriptorCluster);
